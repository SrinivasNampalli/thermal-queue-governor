...
**[Watch the captioned 3D walkthrough](https://srinivasnampalli.github.io/thermal-queue-governor/bridge-watch.html)** · [Download the MP4](demo/media/bridge-walkthrough.mp4). The recording captures the first 3D edition, before the exploded callout guide was added, through actual browser interactions, including the reconstructed 130 °C example and the false 85 °C guard-bypass result. The motor and circuits are explanatory geometry; no physical device was tested. The [recording receipt](demo/media/bridge-recording.json) pins the captured source revision and its page, geometry, data and media hashes.
...
Published Git history is retained at the maintainer's request. Earlier commits still contain the original ZIP and generated copies, so removing them from the current tree does not reduce a full clone's historical transfer size.
