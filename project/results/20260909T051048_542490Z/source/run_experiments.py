...
def main():
    ...
    out=args.out or BASE/"results"/stamp
    ...
    fields=["scenario","seed","policy","k","true_T_C","reading_C","requested",
        "accepted","applied","acknowledged","lo_C","hi_C","valid","reason",
            ...
            for seed in config["seeds"]:
                ...
                for policy in config["policies"]:
                    ...
                    for k in range(n):
                        ...
                        actual=1.0 if scenario=="queue_mismatch" and 250<=k<275 else expected
                        ...
                        row=dict(scenario=scenario,seed=seed,policy=policy,k=k,true_T_C=m.ambient+x,
                            ...
                            applied=actual,acknowledged=expected,lo_C=m.ambient+result["lo"],hi_C=m.ambient+result["hi"],
    ...
    (BASE/"results"/"LATEST.txt").write_text(str(out.relative_to(BASE)))
