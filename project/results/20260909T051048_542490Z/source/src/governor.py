...
def admissible(hi, pending, demand, m):
    ...
    The certificate is consumed for one decision only (not a wall-clock lease).
    """
    ...
    action = min(demand, math.floor((cap+1e-12)/m.step)*m.step, 1.0)
...
class Governor:
    ...
    def decide(self, reading, pending, demand):
        ...
        if reading is not None:
            ...
            self.nominal=max(0.0,reading-self.m.ambient)
        if self.policy=="fixed_cap":
            ...
            midpoint=Model(amin=(self.m.amin+self.m.amax)/2,amax=(self.m.amin+self.m.amax)/2,
