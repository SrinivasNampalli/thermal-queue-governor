...
class Governor:
    ...
    def decide(self, reading, pending, demand):
        ...
        try:
            ...
            return {"action":0.0,"valid":False,"reason":self.reason,"peak":self.hi,
                    "lo":self.lo,"hi":self.hi,"decision_id":self.decision_id}
        if self.policy=="interval_exact_clip" and reading is not None and reading>=self.m.clip-1e-9:
            # Deliberately incorrect ceiling-as-exact ablation, not a valid interval.
            self.hi=min(self.hi,self.m.clip-self.m.ambient+self.m.eps)
        ...
        elif self.policy in ("interval_no_queue","interval_queue","interval_exact_clip"):
            q=[] if self.policy=="interval_no_queue" else pending
