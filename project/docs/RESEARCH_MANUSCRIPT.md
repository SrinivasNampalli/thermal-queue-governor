...
A post hoc reanalysis groups the six scenarios by their 20 paired seeds. The mean is 90.3721%; the sample SD of the 20 seed means is 5.3024 percentage points, with a range of 78.3281-94.4413%. A deterministic paired cluster bootstrap (20,000 resamples, retaining both controllers and all six scenarios within each sampled seed) gives a nominal 95% percentile interval of 88.0190-92.4781%. The paired gain over the fixed cap is 23.2526 percentage points, with an interval of 20.8995-25.3586. The fixed-cap effort ratio is identical across seeds in this design. These intervals describe the selected synthetic design; they do not include hardware variability, model error, alternative scenario weights, or field failure risk. The method and hashes are in [PRIMARY_EVIDENCE.md](../research/PRIMARY_EVIDENCE.md), generated by `tools/analyze_primary_evidence.py`.

The maximum stored primary TQG temperature is 104.99997190667195 C, just 0.00002809332805 C below the 105 C limit. This is a numerical boundary result, not a physical safety margin or sensor accuracy claim. The runner labels a breach above limit + 1e-7 C; even a strict comparison finds no primary TQG maximum above 105 C. Neither the small numerical gap nor the sampled count validates intersample temperatures, calibration, thermal gradients, or shutdown hardware.
